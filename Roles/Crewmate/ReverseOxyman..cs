...
public class ReverseOxyman : RoleBase
{
    ...
    private byte ReverseReverseOxymanId;
    ...
    {
        var id = 647700;
        Options.SetupRoleOptions(id++, TabGroup.CrewmateRoles, CustomRoles.ReverseOxyman);
        (List<Level> trueList, List<Level> falseList) = Enum.GetValues<Level>().Without(Level.None).Split(x => x <= Level.Slow);

        trueList.ForEach(x => LevelSettings[x] = new IntegerOptionItem(++id, $"ReverseOxyman.{x}.BelowPercentage", new(0, 100, 1), x == Level.Blind ? 10 : 30, TabGroup.CrewmateRoles)
            .SetParent(Options.CustomRoleSpawnChances[CustomRoles.ReverseOxyman])
            .SetValueFormat(OptionFormat.Percent));

        falseList.ForEach(x => LevelSettings[x] = new IntegerOptionItem(++id, $"ReverseOxyman.{x}.AbovePercentage", new(0, 100, 1), x == Level.Fast ? 60 : 80, TabGroup.CrewmateRoles)
            .SetParent(Options.CustomRoleSpawnChances[CustomRoles.ReverseOxyman])
            .SetValueFormat(OptionFormat.Percent));

        IncrementByVenting = new IntegerOptionItem(++id, "ReverseOxyman.IncrementByVenting", new(0, 100, 1), 7, TabGroup.CrewmateRoles)
            .SetParent(Options.CustomRoleSpawnChances[CustomRoles.ReverseOxyman])
            .SetValueFormat(OptionFormat.Percent);

        DecreasementEachSecond = new IntegerOptionItem(++id, "ReverseOxyman.DecreasementEachSecond", new(0, 10, 1), 1, TabGroup.CrewmateRoles)
            .SetParent(Options.CustomRoleSpawnChances[CustomRoles.ReverseOxyman])
            .SetValueFormat(OptionFormat.Percent);
    ...
    {
        if (seer.PlayerId != target.PlayerId || seer.PlayerId != ReverseOxymanId || (seer.IsModdedClient() && !hud)) return string.Empty;
